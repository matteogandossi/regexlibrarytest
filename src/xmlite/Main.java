...
public class Main {
	...
	private static void autogeneratedFalseFile() {
		
		String title = Grafica.getFileName("Inserisci l'argomento del file.");
		String regex = Grafica.getRegex("Inserisci la regex");
		
		RegExp reg = new RegExp(regex);		
		RegExp univ = new RegExp("[\\u003f-\\u0126]*");
		Automaton univA = univ.toAutomaton();
		Automaton complA = reg.toAutomaton().complement();
		...
		Generex g = new Generex(intersA);
		
		for(int i=0; i<size; i++) {
			
			input = g.random((i+1)*4, (i+1)*4);
			newItem = new Item(input, false);
			list.add(newItem);
		}
